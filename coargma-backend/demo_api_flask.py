...
def get_result_obj_asp_ru():
    obj1 = str(request.args.get('obj1'))
    obj2 = str(request.args.get('obj2'))
    asp = str(request.args.get('asp'))
    top = int(request.args.get('top', default=10))
    use_baseline = request.args.get('use_baseline', type=str)
    if use_baseline.lower() == 'true':
        use_baseline = True
    elif use_baseline.lower() == 'false':
        use_baseline = False
    return jsonify(dab_ru.get_result_on_objs_asp_ru(obj1=obj1, obj2=obj2, aspect=asp, top=top, use_baseline=use_baseline))
...
def get_result_ru():
    ...
    if use_baseline.lower() == 'true':
        ...
        use_baseline = False
    return jsonify(dab_ru.get_result_on_question_ru(question=question, top=top, use_baseline=use_baseline))
